...
//the message to be signed //the length of message
// ZA=Hash(ENTLA|| IDA|  a|| b|| Gx || Gy || xA | yA) //a random number K lies in [1,n-1]
//the private key
...
ZA         //ZA=Hash(ENTLA|| IDA | a | b|| Gx || Gy || xA|
