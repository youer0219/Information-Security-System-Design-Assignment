...
- TODO:目前的实现中没有验证`next`，所以可能遭到[重定向攻击](https://www.doubao.com/thread/w1b0d1ad7f2000db4)。
    - 这个需要等到权限系统设计好后才好验证next是否合法。所以暂时不管。
...
- 为了安全起见，特定操作应该登录特定用户的数据库账号，并且限制其权限。
    - 对数据库的操作与对应的最小权限
        - 登录： 只允许读取用户表  `TODO: 限制登录时的链接权限为只可读取用户表。`
        - 个人资料修改：只允许修改特定字段
        - 账号注册
    - `TODO: 完善数据库操作总结`
...
- 简化`公文建模`部分的公文设计，实际设计如下：
    - [AI回答](https://lxblog.com/qianwen/share?shareId=8a75e5a0-1c4d-4ca0-8b73-b5be5e29aa71)
        ...
        content - 公文正文内容，不能为空。
        author_id - 作者ID，不能为空。  `这里不设置关联，因为不可能删除用户名后把公文也删掉。`
...
- 需要的功能：
    - 公文列表： 查找公文 只可读
        ...
        - TODO:同时检查权限，不展示用户不可查看的公文。(目前没有权限系统，都展示)
...
- 在Windows下配置gmssl-python库，由此可以使用其生成SM2密钥对的功能
    - 安装cmake
    - 安装Windows的“make”
    - 下载编译gmssl库（动态库）
    - 安装gmssl-python
...
- 如何加解密：
    - 在与数据库交互过程中完成加解密。这部分交互代码都写在公文数据库管理类中。
    - 本地加解密的方案是密钥加密密钥--工作密码的模式。
    - 公文签收则需要SM2公私钥对来辅助
    - 之所以安装gmssl-python，是因为想要尽可能不通过龙脉硬件处理数据。以及在不使用芯片前先进行测试。
    - 但同时也应该测试硬件的加解密/签名验证功能了
        - 已测试SM2签名
        - 需要测试：SM4解密
            - 建议换成私钥解密，工作密钥是SM4密钥就好
        - 以及如何保存一个公私钥对与SM4密钥
...
- 测试龙脉硬件的密钥应用
    ...
    - 私钥解密暂时无法完成。可能的替代方法就是把SM4密钥保存到另一个数据库中，至少不放在一起。
